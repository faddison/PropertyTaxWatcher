...
public class PropertyTaxWatcher implements EntryPoint {
	...
	private List<PropertyTax> propertyTaxes;
	...
	public void runVisualizations() {
		VisualizationUtils.loadVisualizationApi(new Runnable() {
			public void run() {
				...
				int[] columnIndices = { 12 }; // postal code
				mapView.setColumns(columnIndices);
	...
	private Widget createMapVisualization(DataView dataView) {
		MapVisualization mapVis = new MapVisualization("400", "400");
		mapVis.draw(dataView);
		mapVis.draw(dataView);
		return mapVis.asWidget();
	}
